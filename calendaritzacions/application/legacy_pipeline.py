...
import pandas as pd
import os
from calendaritzacions.convert import xlsx_to_csv
import shutil
...
def processar_dades_2(df, nom_fitxer="dades.csv", task_id=None, segona_fase_bool=False):
    ...
    input_demand_analysis = build_input_demand_analysis(df)
    ...
    kpis_payload = build_kpis_payload(
        ...
        input_demand_analysis=input_demand_analysis,
    )

    write_kpis_json(kpis_path, kpis_payload)
    input_demand_plots = {}
    try:
        input_demand_plots = write_input_demand_plots(
            input_demand_analysis,
            input_demand_plots_dir,
            stem=f"input_demand_{nom_fitxer}",
        )
