...
class ResourceSolverDecompositionPlotTests(unittest.TestCase):
    ...
    def test_decomposition_plots_create_manifest_and_files_for_multiple_components(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("initial_hub_concentration_heatmap", manifest["plots"])
            self.assertIn("hub_cut_network_C001", manifest["plots"])
            self.assertIn("hub_cut_ranking_C001", manifest["plots"])
            self.assertIn("hub_cut_matrix_C001", manifest["plots"])
            ...
            self.assertIn("initial_hub_concentration_heatmap", manifest["plot_descriptions"])
            self.assertIn("hub_cut_network_C001", manifest["plot_descriptions"])
            self.assertTrue(Path(manifest["hub_cut_diagnostics"]).exists())
            diagnostics = json.loads(Path(manifest["hub_cut_diagnostics"]).read_text(encoding="utf-8"))
            self.assertEqual(diagnostics["artifact_type"], "resource_solver_hub_cut_diagnostics")
            self.assertGreaterEqual(diagnostics["hub_count"], 1)
            self.assertIn("best_cut_nodes", diagnostics["hubs"][0])
